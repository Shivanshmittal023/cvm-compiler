...
std::string NumberExpr::dump(int indent) const {
    return getIndent(indent) + "NumberExpr(" + std::to_string(value) + ")\n";
}
...
std::string CallExpr::dump(int indent) const {
    ...
    if (!arguments.empty()) {
        ...
        for (const auto& arg : arguments) ss << arg->dump(indent + 2);
    }
    return ss.str();
}
...
std::string BlockStmt::dump(int indent) const {
    ...
    for (const auto& stmt : statements) ss << stmt->dump(indent + 1);
    ss << getIndent(indent) << "]\n";
    return ss.str();
}
...
std::string ReturnStmt::dump(int indent) const {
    ...
    if (returnValue) ss << returnValue->dump(indent + 1);
    return ss.str();
}
...
std::string VarDeclStmt::dump(int indent) const {
    std::stringstream ss;
    ss << getIndent(indent) << "VarDeclStmt(type: '" << varType << "', name: '" << name << "')\n";
    if (initializer) {
...
std::string FunctionDecl::dump(int indent) const {
    std::stringstream ss;
    ss << getIndent(indent) << "FunctionDecl(returnType: '" << returnType << "', name: '" << name << "')\n";
    if (!parameters.empty()) {
        ss << getIndent(indent + 1) << "[Parameters]:\n";
        for (const auto& p : parameters) {
            ss << getIndent(indent + 2) << "Type: " << p.first << ", Name: " << p.second << "\n";
        }
    }
    ss << getIndent(indent + 1) << "[Body]:\n" << body->dump(indent + 2);
    return ss.str();
...
std::string Program::dump(int indent) const {
    ...
    for (const auto& decl : declarations) ss << decl->dump(indent + 1);
    return ss.str();
}
